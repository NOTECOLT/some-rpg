...
#define cJSON_GetItem cJSON_GetObjectItemCaseSensitive 
...
int BUFFER_LEN = 16384; // 2^14 bytes

// FUNCTIONS
//------------------------------------------------------------------------------------------

/** Loads a map from a JSON path. */
Map* LoadMap(char* path) {
	// READ PATH
	FILE *fp = fopen(path, "r");

	if (fp == NULL) {
		printf("[MAPLOADER] path %s does not exist! Cannot open file.", path);
		return NULL;
	}

	printf("[MAPLOADER] Loading Map at path %s.", path);

	char buffer[BUFFER_LEN];
	int len = fread(buffer, BUFFER_LEN, 1, fp);
	fclose(fp);
	...
	//map->texture = malloc(sizeof(Texture2D));

	cJSON* item;

	// Name
	item = cJSON_GetItem(json, "name");
	if (cJSON_IsString(item) && item->valuestring != NULL) {
		char* str = item->valuestring;
		map->name = malloc(sizeof(str)+1);
		strcpy(map->name, str);
	}

	// Tilemap
	item = cJSON_GetItem(json, "tilemap");
	if (cJSON_IsString(item) && item->valuestring != NULL) {
		Texture2D tex = LoadTexture(item->valuestring);
		map->texture = malloc(sizeof(tex));
		map->texture = &tex;
	}

	// Size
	map->size = malloc(sizeof(Vector2));
	item = cJSON_GetItem(json, "width");
	if (cJSON_IsNumber(item)) {
		map->size->x = item->valuedouble;
	}

	item = cJSON_GetItem(json, "height");
	if (cJSON_IsNumber(item)) {
		map->size->y = item->valuedouble;
	}
...
/**Frees map struct and all its contents*/
